angular.module('umbraco.services').config(['$httpProvider', function ($httpProvider) {
    ...
    $httpProvider.interceptors.push(function ($q) {
        return {
            'response': function (response) {
				...
				if (response.config.url.indexOf('views/components/application/umb-login.html') > -1) {
    ...
    function loadLoginPage(response) {
        ...
        html.find("#login h1")
            .html("Welcome to <span class='cam-green'>Umbraco CMS</span> <span class='cam-basedon'>- Maintained by <span class='cam-orange'>Camelonta AB</span></span>")
            .css('opacity', 1);
